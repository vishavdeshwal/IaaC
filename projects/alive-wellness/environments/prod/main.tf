terraform {
  ...
  backend "s3" {
    ...
    key     = "alive-wellness/prod/terraform.tfstate"
    region  = "ap-south-1"
    encrypt = true
...
module "ecs_backend_worker" {
  ...
  container_definitions = jsonencode([
    {
      name      = "backend-worker"
      image     = "544433947816.dkr.ecr.ap-south-1.amazonaws.com/prod-alive-wellness-alive-wellness-backend:dummy"
...
resource "aws_lb_listener_rule" "strapi" {
  ...
  condition {
    host_header {
      values = ["cms.skinverse.in"] # e.g. cms.alivewellness.com
    }
  }
}
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        { name = "API_URL", value = "https://saleor.skinverse.in/graphql/" }
      ]
    }
  ])
}
